...
function parseExtract(tokens: string[]): ExtractCommand {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    switch (flag.name) {
      ...
      case 'fit': {
        const book = requireBook('--fit');
        // Like --orientation, --fit only affects the PDF target; routed through
        // applyAction with no targets so it survives the implicit-stdout reset.
        applyAction(book, { targets: [], fit: parseSwitchBool(flag.suffix, '--fit') });
        sheetAllowed = false;
        break;
      }
      case 'xml': {
